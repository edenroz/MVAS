...
//this inclusion is for integrating intercept and multi-view kernel modules
#include "../multi-view/fault_types.h"
...
extern int multi_view_page_fault(struct pt_regs* regs, long error_codei, long realign_mode);
...
//this part has been added for multi view management
...
void my_do_page_fault(struct pt_regs* regs, unsigned long error_code){
    ...
    void* target_address;
    char aux, aux1;
   // printk(KERN_INFO "y_virt_drv: page fault detected in process %lu.\n", (unsigned long)task->pid);
    before_image = *regs;


    // this is for integration with the multi-view module
    discriminate = multi_view_page_fault(regs,error_code,0);

    // this is for integration with the multi-view module
    if(discriminate == NORMAL_FAULT ){
//	printk("calling a real page fault for process %d\n",current->pid);
         ((do_page_fault_t)addr_dft_do_page_fault)(regs, error_code);
	//printk("done a real page fault for process %d - discriminate is %d (relaoding the parallel view)\n",current->pid,discriminate);

	return;

    }

    if(discriminate == WATCHDOG_NORMAL_FAULT){
	printk("calling a watchdog real page fault for process %d\n",current->pid);
         ((do_page_fault_t)addr_dft_do_page_fault)(regs, error_code);
    	printk("done a watchdog real page fault for process %d - discriminate is %d (relaoding the parallel view)\n",current->pid,discriminate);
	return;
//    	after_image = *regs;
//	printk("process %d - reg comparison after watchdog real do_page_fault is %d\n",current->pid,memcmp(&before_image,&after_image,sizeof(struct pt_regs)));
    }

    if(discriminate == WATCHDOG_FICTITIOUS_FAULT){
//	printk("returning from a multiview fault for process %d\n",current->pid);
//	my_native_write_cr3(__pa(pdg_addr[i]));
//	discriminate = root_sim_page_fault(regs,error_code,1);
//	printk("additional call to the custom fault handler - return should be 45 (realign only cr3) - it is %d\n",discriminate);
